 ...
 * this repo builds standalone with no harness project references.
 *
...
/**
 * Node-half externals: the DSH host loader resolves every `@deepseek-ai/*`
 * value import through the DSH install's module table (like `cordis`), so
 * they stay external instead of being inlined from the install layout.
 */
...
function clientConfig(id: string): UserConfig {
  return {
    ...
    // loader module table must inline instead (wire/type layers, clsx —
    // every non-shared dep). A require() the table cannot answer is
