...
// v1.4.0 冒险扩展（单向依赖：events/relics 不 import engine，无循环）
import { comboStance, EVENT_EVERY_TURNS, rollEvent, tickEvents, type EventEffectDef } from './events.ts'
...
export const STREAK_REWARDS: Record<number, { xp: number }> = {
  3: { xp: 50 },
  7: { xp: 150 },
  14: { xp: 300 },
  30: { xp: 800 },
...
export const SEASON_PASS_TIERS: { id: string; seasonXp: number; xp: number }[] = [
...
export function rollDailyQuests(now: number, salt = ''): DailyQuestState {
...
export function buyShopItem(
  ...
  now: number = Date.now(),
  seasonOverride?: string,
): { ok: boolean; reason?: string; save: SaveData } {
  ...
  if (balance < item.price) return { ok: false, reason: 'insufficient-balance', save: s }
 ...
 * 库存不足 / 全部已完成 → { ok: false }。
 */
export function useQuestSkip(save: SaveData, now: number = Date.now()): { ok: boolean; save: SaveData } {
  const s = structuredClone(save)
  const shop: ShopState = { ...freshShop(), ...(s.shop ?? {}) }
  if ((shop.questSkips ?? 0) <= 0) return { ok: false, save: s }
  const daily = ensureDaily(s, now)
  const target = daily.quests.find(q => !q.done)
  if (target === undefined) return { ok: false, save: s }
...
export function claimPassTier(
  ...
  if (tier === undefined) return { ok: false, gained: 0, save: structuredClone(save) }
  const s = structuredClone(save)
  const shop: ShopState = { ...freshShop(), ...(s.shop ?? {}), passClaimed: s.shop?.passClaimed ?? [] }
  if (shop.passClaimed!.includes(tier.id)) return { ok: false, gained: 0, save: s }
  if (s.player.seasonXp < tier.seasonXp) return { ok: false, gained: 0, save: s }
...
export function useReroll(save: SaveData, now: number = Date.now()): { ok: boolean; save: SaveData } {
  const s = structuredClone(save)
  const shop: ShopState = { ...freshShop(), ...(s.shop ?? {}) }
  if (shop.rerolls <= 0) return { ok: false, save: s }
  shop.rerolls -= 1
  s.shop = shop
...
export function checkTutorial(
  ...
  now: number = Date.now(),
  seasonOverride?: string,
): { stepIds: string[]; complete: boolean; save: SaveData } {
  const s = structuredClone(save)
  const tutorial: TutorialState = s.tutorial ?? { steps: {}, done: false }
...
export const SHOP_THEMES: Record<string, string> = {
  'theme-ember': 'ember',
  'theme-frost': 'frost',
...
/** 新手链专属称号（全部完成解锁）。 */
export const TUTORIAL_TITLE = { zh: '见习冒险者', en: 'Rookie Adventurer' } as const
...
export function addXp(save: SaveData, gain: number, now: number = Date.now(), seasonOverride?: string): SaveData {
  ...
  if (c.lastActiveDay !== ''
    ...
    if (gapDays >= 3) gain += 100 + Math.min(200, gapDays * 10) // 100 + 每离线一天 +10（封顶 +200）
  }
  if (c.lastActiveDay !== today) {
    ...
    if ((c.streakBest ?? 0) < c.streakDays) {
      ...
      if (tier !== undefined) gain += tier.xp
    }
  }
  if (gain > 0) {
    s.player.xp += gain
  ...
  if (gain > 0) {
    const history: Record<string, DayHistory> = s.history ?? {}
...
export function trimHistory(history: Record<string, DayHistory>, now: number): Record<string, DayHistory> {
  const cutoff = dayKey(now - HISTORY_KEEP * 86_400_000)
  const out: Record<string, DayHistory> = {}
  for (const [date, h] of Object.entries(history)) {
    if (date >= cutoff) out[date] = h
  }
  return out
}
...
export function applyTurnDetailed(
  ...
  // v1.4.0：tokens/todo 的 XP 单独记账（事件/姿态倍率在组合阶段统一应用）
  for (const a of actions) {
    switch (a.kind) {
      ...
      case 'subagent':
        ...
        break
      default:
        gain += xpForAction(a) // turn-completed +10 / turn-failed +2
        break
    }
  }
  ...
  // v1.4.0 事件效果核算（护盾/清零/倍率；在完成判定后调用——窗口消费依赖 completed）。
  ...
  if (completed) {
    ...
    if (h >= 0 && h < 5) c.nightTurns++ // 凌晨回合（night_owl_10 / dq_night_1 用）
    const today = dayKey(now)
    ...
    c.turnsFailed++
    // 连击保险：失误回合自动消耗一个，连击不清零（商店购买 / v1.4.0 幽灵 Bug 事件护盾）。
    if (mods.shieldFailure || (s.shop?.shields ?? 0) > 0) {
      if (!mods.shieldFailure) {
  ...
  // v1.4.0 随机事件触发：每 EVENT_EVERY_TURNS 个完成回合掷一次「命运骰子」。
  // 注意：rollEvent 返回 clone，替换 s 前所有计数器更新必须已落定（连击块在上面）。
  let eventCard: { id: string; def: EventEffectDef } | null = null
  if (completed) {
    c.turnsSinceEvent = (c.turnsSinceEvent ?? 0) + 1
    if ((c.turnsSinceEvent ?? 0) >= EVENT_EVERY_TURNS) {
      const rolled = rollEvent(s, now, `${dayKey(now)}-${c.turnsCompleted}`)
      if (rolled !== null) {
        s = rolled.save
        eventCard = { id: rolled.id, def: rolled.def }
      }
    }
  }

  // v1.4.0 连击姿态：≥10/25/50/100 解锁（工具 XP 追加 + tokens 倍率）。
  const stance = completed ? comboStance(c.consecutiveSuccess) : null

  // 组合本轮 XP：工具（封顶 10 + 事件/姿态追加——仅当本回合确有工具调用）+ tokens（事件×姿态倍率）+ todo（事件×2）+ 回合基础
  const toolBonus = toolGain > 0 ? mods.toolBonus + (stance?.toolBonus ?? 0) : 0
  let gainTotal = Math.min(toolGain, 10) + toolBonus
    + Math.round(tokenXp * mods.tokenMultiplier * (stance?.tokenMultiplier ?? 1))
    + (mods.todoBonus ? todoXp * 2 : todoXp)
    + gain
  // 连击多档加成：≥5 ×1.5，≥15 ×2.0，≥30 ×2.5。
  if (completed) {
    if (c.consecutiveSuccess >= 30) { gainTotal = Math.round(gainTotal * 2.5); combo = 2.5 }
    else if (c.consecutiveSuccess >= 15) { gainTotal = Math.round(gainTotal * 2.0); combo = 2.0 }
    else if (c.consecutiveSuccess >= 5) { gainTotal = Math.round(gainTotal * 1.5); combo = 1.5 }
  }

  // 经验加成卡：剩余回合内 XP +50%（v1.3.3：0 XP 空回合不消耗加成次数）。
  ...
  // v1.4.0 技术债上门：本轮产出清零（愤怒的债主；任务奖不受影响）。
  ...
  // v1.4.0 圣物掉落：每轮 todo 全清 5% 概率（多轮累加封顶 30%）。
  let relicDropped: string | null = null
  if (sweeps > 0) {
    const rr = rollRelic(withRecords, Math.min(0.05 * sweeps, 0.3), now, `${dayKey(now)}-r${c.turnsCompleted}`)
    withRecords = rr.save
  ...
  return {
    ...
    settlement: {
      ...
      turnsDone,
      ...(eventCard !== null ? { eventCard: { id: eventCard.id, defId: eventCard.def.id } } : {}),
...
export function migrateSave(raw: Partial<SaveData>, cwd: string, seasonOverride: string | undefined): SaveData {
  ...
  const out: SaveData = {
    ...
    shop: (() => {
      ...
      const rawShop: Partial<ShopState> = raw.shop ?? {}
 ...
 * - 等级从累计 XP 重算
 */
export function mergeSaves(saves: SaveData[], now: number = Date.now()): SaveData {
  ...
  for (const s of saves) {
    ...
    for (const tool of sc.todayTools) {
      if (!c.todayTools.includes(tool)) c.todayTools.push(tool)
    }
  ...
  if (latest.counters.lastSuccessAt !== undefined) c.lastSuccessAt = latest.counters.lastSuccessAt
  // 等级从累计 XP 重算
  let level = 1
