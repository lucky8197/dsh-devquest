 ...
 */
import type { ActiveEvent, Counters, SaveData } from './types.ts'
...
export const EVENT_POOL: EventEffectDef[] = [
  // ---- buff（增益）----
  {
...
export function pendingEvent(save: SaveData): boolean {
 ...
 */
export function rollEvent(save: SaveData, now: number, seed: string): { id: string; def: EventEffectDef; save: SaveData } | null {
...
export function resolveEvent(
  ...
  now: number,
  seed: string,
): { ok: boolean; gained: number; label: string; save: SaveData } {
  const s = structuredClone(save)
  const ev = (s.events ?? []).find(e => e.id === eventId)
  const fail = (label: string): { ok: boolean; gained: number; label: string; save: SaveData } => ({ ok: false, gained: 0, label, save: s })
  if (ev === undefined) return fail('事件不存在或已过期')
  const def = EVENT_POOL.find(d => d.id === ev.effectId)
  if (def === undefined || def.kind !== 'choice') return fail('不可结算')
  ...
  switch (def.id) {
    case 'ev-easteregg': {
      ...
      const buff: ActiveEvent = { id: `evx-${now}`, effectId: 'ev-relicluck', gainedAt: now, expiresAt: now + 60 * 60_000 }
    ...
    case 'ev-midnight': {
      if (option === 0) {
        ...
        const buff: ActiveEvent = { id: `evx-${now}`, effectId: 'ev-doublestreak', gainedAt: now }
        s.events = [...(s.events ?? []), buff]
    ...
    case 'ev-gamble': {
      ...
      if (win) return { ok: true, gained: 200, label: '🎲 大成功：+200 XP', save: s }
...
export const COMBO_STANCES: ComboStanceDef[] = [
...
export function comboStance(consecutive: number): ComboStanceDef | null {
  let best: ComboStanceDef | null = null
  for (const s of COMBO_STANCES) if (consecutive >= s.combo) best = s
  return best
}

/** 是否刚达成新的姿态档位（client 全屏特效用：上一连击 < combo ≤ 当前）。 */
export function stanceJustReached(prevCombo: number, nowCombo: number): ComboStanceDef | null {
  const now = comboStance(nowCombo)
  if (now === null) return null
  const prev = comboStance(prevCombo)
  if (prev !== null && prev.combo >= now.combo) return null
  return now
}

// ---------------------------------------------------------------------------
// 回合结算钩子（engine.applyTurnDetailed 调用）
// ---------------------------------------------------------------------------
...
export function tickEvents(
  ...
  const mods: TurnEffectMods = { toolBonus: 0, tokenMultiplier: 1, gainMultiplier: 1, shieldFailure: false, wipeGain: false, todoBonus: false }
  ...
  for (const e of live) {
    ...
    switch (def.id) {
      ...
      case 'ev-refactor':
        if (completed) mods.toolBonus -= 1 // 工具 XP 减半的近似（基础 2 → 1）
        break
      case 'ev-techdebt':
        e.consumed = true
        mods.wipeGain = true
...
export function hasFailureShield(events: ActiveEvent[] | undefined): boolean {
  return (events ?? []).some(e => e.effectId === 'ev-ghostbug' && e.consumed !== true)
}

/** 技术债上门：本轮结算清零（一次性）。 */
export function techDebtConsumed(events: ActiveEvent[] | undefined): boolean {
  return (events ?? []).some(e => e.effectId === 'ev-techdebt' && e.consumed === true)
}

/** 圣物掉率 ×2 是否生效（神秘彩蛋保留选项）。 */
export function relicLuckActive(events: ActiveEvent[] | undefined, now: number): boolean {
  return (events ?? []).some(e => e.effectId === 'ev-relicluck' && (e.expiresAt === undefined || now < e.expiresAt))
}

/** 明日连击阶梯 ×2 标记是否在场（凌晨抉择：继续肝）。 */
export function doubleStreakActive(events: ActiveEvent[] | undefined): boolean {
  return (events ?? []).some(e => e.effectId === 'ev-doublestreak')
}

/** 消费「明日连击 ×2」标记（addXp 发放阶梯奖励时调用并叠加一次）。 */
export function consumeDoubleStreak(save: SaveData): SaveData {
  const s = structuredClone(save)
...
export function todoBonusActive(events: ActiveEvent[] | undefined, now: number): boolean {
