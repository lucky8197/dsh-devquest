...
*/
function applyTurn(save, actions, now = Date.now(), seasonOverride) {
	const s = structuredClone(save);
	const c = s.counters;
	const season = seasonOverride ?? autoSeasonId(now);
	if (s.player.season !== season) {
...
async function loadSave(ctx, config, cwd) {
	...
	try {
		...
		if (await ctx.fs.stat(target) === void 0) return freshSave(scopeKey(cwd), config.season);
		const text = await ctx.fs.readText(target);
		return migrateSave(JSON.parse(text), scopeKey(cwd), config.season);
	} catch (error) {
