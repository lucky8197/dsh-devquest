...
export function renderStatus(status, detail) {
    ...
    // v1.3.0 职业画像
    if (status.class !== null)
        lines.push(`   🃏 职业: ${status.class.icon} ${status.class.name.zh}`);
    // v1.3.0 赛季结算（换季自动生成）
    if (status.seasonSummary !== undefined) {
        lines.push(`   📜 上赛季 ${status.seasonSummary.season}：Lv.${status.seasonSummary.level} · 最高连击 ${status.seasonSummary.comboBest} · ${status.seasonSummary.seasonXp} XP · ${status.seasonSummary.achievements} 枚成就`);
    }
    // v1.3.0 每日 XP 目标
    const dg = status.dailyGoal;
    if (dg !== undefined && dg.goal > 0) {
    ...
    if (weeklyQuests.length > 0) {
        ...
        if (status.weekly.bonusReady)
            lines.push(`     🎁 周全清奖励可领取（+100 XP）`);
        // v1.3.0 每周 BOSS
        const boss = status.weekly.boss;
        if (boss.name !== '') {
            lines.push(`     🐉 每周 BOSS「${boss.name}」${boss.claimed ? '已击败 ✓' : `${Math.min(boss.damage, boss.hp)}/${boss.hp}${boss.defeated ? `（可领取 +${boss.reward} 货币）` : ''}`}`);
        }
    }
    // 商店 + 当前主题皮肤
    ...
    ctx.tools.register(defineTool({
        ...
        async execute() {
            ...
            if (daily.length > 0) {
                ...
                if (status.dailyChest?.ready === true)
                    lines.push('  🎁 全清宝箱可领取 +50 XP！');
            }
            // 每周挑战
            ...
            if (weekly.length > 0) {
                ...
                if (status.weekly.bonusReady === true)
                    lines.push('  🎁 全清周奖励可领取 +100 XP！');
                // v1.3.0 每周 BOSS
                const boss = status.weekly.boss;
                if (boss.name !== '') {
                    lines.push(`🐉 每周 BOSS「${boss.name}」${boss.claimed ? '已击败 ✓' : `${Math.min(boss.damage, boss.hp)}/${boss.hp}（${boss.defeated ? `可领取 +${boss.reward} 赛季货币！` : '全清周挑战击败'}`}`);
                }
            }
            // v1.3.0 每日 XP 目标
            const dg = status.dailyGoal;
            if (dg !== undefined && dg.goal > 0) {
                lines.push(`🎯 今日目标 ${Math.min(dg.todayXp, dg.goal)}/${dg.goal} XP${dg.claimed ? '（已达成 ✓）' : dg.todayXp >= dg.goal ? `（可领取 +${dg.rewardXp} XP！）` : ''}`);
            }
            if (status.lucky?.available === true)
                lines.push('🎁 今日幸运抽奖可抽！');
