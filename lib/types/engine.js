...
import { comboStance, EVENT_EVERY_TURNS, rollEvent, tickEvents } from "./events.js";
import { rollRelic } from "./relics.js";
/** 称号（每 5 级一档）。 */
export const TITLES = [
...
export function useReroll(save, now = Date.now()) {
    ...
    for (const q of next.quests) {
        ...
        const def = DAILY_QUEST_POOL.find(d => d.id === q.id);
        if (def !== undefined)
            q.base = def.progress(s.counters);
    }
    // 宝箱领取状态保留（重掷不放宽当天宝箱门）
...
// ---------------------------------------------------------------------------
/** 每日目标可设定的档位（XP）。 */
export const DAILY_GOAL_OPTIONS = [200, 400, 800, 1500];
/** 每日目标达成奖励 XP。 */
export const DAILY_GOAL_REWARD = 50;
/** 今日已获 XP（跨天自动归零）。 */
export function todayXpOf(save, now = Date.now()) {
    const c = save.counters;
    return c.todayXpDay === dayKey(now) ? (c.todayXp ?? 0) : 0;
}
/** 设定每日 XP 目标（0=关闭）。 */
export function setDailyGoal(save, goal, now = Date.now()) {
    const s = structuredClone(save);
 ...
 * 返回是否成功与领取的 XP。
 */
export function claimDailyGoal(save, now = Date.now(), seasonOverride) {
    const s = structuredClone(save);
    const goal = s.player.dailyGoal ?? 0;
    const today = dayKey(now);
    if (goal <= 0)
        return { ok: false, gained: 0, save: s };
    if (s.player.dailyGoalClaimedDay === today)
        return { ok: false, gained: 0, save: s };
    if (todayXpOf(s, now) < goal)
        return { ok: false, gained: 0, save: s };
...
/** 合成本周 Boss（纯函数）。 */
export function computeWeeklyBoss(save, now = Date.now()) {
    const weekly = ensureWeekly(save, now);
    if (weekly === undefined)
        return null;
 ...
 */
export function claimWeeklyBoss(save, now = Date.now()) {
    const s = structuredClone(save);
    const weekly = s.weekly;
    if (weekly === undefined || weekly.week !== weekKey(now))
        return { ok: false, gained: 0, save: s };
    if (weekly.bossClaimed === true)
        return { ok: false, gained: 0, save: s };
    const doneCount = weekly.quests.filter(q => q.done).length;
    if (doneCount < weekly.quests.length || weekly.quests.length === 0)
        return { ok: false, gained: 0, save: s };
...
export function applyTurnDetailed(save, actions, now = Date.now(), seasonOverride) {
    ...
    if (completed) {
        if (c.consecutiveSuccess >= 30) {
            ...
            gainTotal = Math.round(gainTotal * 2.0);
            combo = 2.0;
        }
...
function sanitizeMigratedNumbers(out, base) {
    ...
    const playerFields = ['level', 'xp', 'xpTotal', 'seasonXp'];
    for (const f of playerFields) {
        const v = outPlayer[f];
        if (typeof v !== 'number' || !Number.isFinite(v))
            outPlayer[f] = basePlayer[f];
    }
    ...
    if (out.shop !== undefined) {
        ...
        const shopFields = ['spent', 'shields', 'rerolls', 'xpBoostTurns', 'questSkips', 'bossEarned'];
        for (const f of shopFields) {
            const v = outShop[f];
            if (typeof v !== 'number' || !Number.isFinite(v))
                outShop[f] = baseShop[f];
        }
    }
}
/**
    ...
    for (const s of saves) {
        ...
        if (shop !== undefined) {
            ...
            for (const p of shop.passClaimed ?? []) {
                if (!(out.shop.passClaimed ?? []).includes(p))
                    out.shop.passClaimed = [...(out.shop.passClaimed ?? []), p];
            }
