 ...
 * 缺省 delayMs = 250（毫秒）。
 */
export function createSaveWriter(ctx, config, delayMs = 250) {
    let latest = null;
    let timer = null;
...
export const DEFAULT_UI_SETTINGS = { fontSize: 1, compact: false, toastFilter: 'all', sound: true, notify: true };
/** UI 设置文件绝对路径。 */
export function settingsPath(config) {
    return join(dataRoot(config), 'settings.json');
}
/** 校验并补全设置对象（未知/越界字段回落默认，保证写出的永远是合法形状）。 */
export function sanitizeUiSettings(raw) {
...
export async function loadUiSettings(ctx, config) {
    try {
...
/** 写 UI 设置（原子替换）。 */
export async function saveUiSettings(ctx, config, settings) {
    const target = await ctx.fs.resolve(settingsPath(config));
