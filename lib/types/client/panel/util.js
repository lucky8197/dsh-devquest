...
export function apiErrorOf(data) {
...
export function updatedLabel(refreshedAt) {
    if (refreshedAt === null)
        return '—';
 ...
 * - 30+   太阳神   炽金橙渐变 + 强光晕
 */
export function titleTone(level) {
    if (level >= 30)
...
export function titleToneStyle(level) {
...
/** 稀有度 → toast 边框样式。 */
export function rarityToastStyle(rarity) {
...
/** 稀有度 → 成就墙已解锁格子光晕。 */
export function rarityCellStyle(rarity) {
    const color = RARITY_COLOR[rarity] ?? RARITY_COLOR.common;
    return {
...
export const DEFAULT_SETTINGS = { fontSize: 1, compact: false, toastFilter: 'all', sound: true, notify: true };
/**
...
/** 稀有度权重（toast 过滤用）。 */
export const RARITY_WEIGHT = { common: 0, rare: 1, epic: 2, legendary: 3 };
/** 稀有度 → 权重。 */
export function rarityWeight(r) {
    return RARITY_WEIGHT[r] ?? 0;
}
/** 限制面板位置：四周至少保留 MIN_VISIBLE 可见，拖不丢。 */
export function clampPanelPos(left, top, width, height) {
