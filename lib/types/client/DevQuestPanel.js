...
const DEFAULT_SETTINGS = { fontSize: 1, compact: false, toastFilter: 'all', sound: true, notify: true };
...
/** 稀有度权重（toast 过滤用）。 */
const RARITY_WEIGHT = { common: 0, rare: 1, epic: 2, legendary: 3 };
/** 稀有度 → 权重。 */
...
export function DevQuestPanelCard(props) {
    ...
    const [wallRarity, setWallRarity] = useState('all');
    const [wallStatus, setWallStatus] = useState('all');
    ...
    const panelMsgTimer = useRef(null);
    ...
    const [questReminderMsg, setQuestReminderMsg] = useState(null);
    ...
    const notify = useCallback((ok, text) => {
        setPanelMsg({ ok, text });
        if (panelMsgTimer.current !== null)
            window.clearTimeout(panelMsgTimer.current);
    ...
    const importSave = useCallback(async (file) => {
        ...
        try {
            ...
            if (data.status !== null && data.status !== undefined)
                actions.setStatus(data.status);
            if (data.ok)
                notify(true, t('dq.imported'));
            else
                notify(false, apiErrorOf(data) ?? t('dq.importFailed'));
        }
        catch {
    ...
    const switchTitle = useCallback(async (titleId) => {
        try {
            ...
            if (data.status !== null && data.status !== undefined)
                actions.setStatus(data.status);
            if (data.ok)
                notify(true, t('dq.titleSwitched'));
            else
                notify(false, apiErrorOf(data) ?? t('dq.opFailed'));
        }
        catch {
    ...
    // v1.3.0 每日目标：设定 / 领取。
    const setGoalF = useCallback(async (goal) => {
        try {
            ...
            if (data.status !== null && data.status !== undefined)
                actions.setStatus(data.status);
            if (data.ok)
                notify(true, goal > 0 ? t('dq.dailyGoalSet') : t('dq.dailyGoalOff'));
            else
                notify(false, apiErrorOf(data) ?? t('dq.opFailed'));
        }
        catch {
    ...
    const claimDailyGoalF = useCallback(async () => {
        try {
    ...
    /** v1.3.0 领取每周 BOSS 掉落。 */
    const claimBossF = useCallback(async () => {
        try {
    ...
    /** 领取赛季通行证档位奖励。 */
    const claimPassTier = useCallback(async (tierId) => {
        try {
            ...
            if (data.status !== null && data.status !== undefined)
                actions.setStatus(data.status);
            if (data.ok)
                notify(true, t('dq.passClaimed', { xp: data.gained }));
            else
                notify(false, apiErrorOf(data) ?? t('dq.opFailed'));
        }
        catch {
    ...
    /** 使用任务跳过卡。 */
    const useQuestSkipCard = useCallback(async () => {
        try {
    ...
    const claimWeekly = useCallback(async () => {
        ...
        try {
            ...
            if (data.status !== null && data.status !== undefined)
                actions.setStatus(data.status);
            if (data.ok)
                notify(true, t('dq.weeklyClaimed', { xp: data.gained }));
            else
                notify(false, apiErrorOf(data) ?? t('dq.opFailed'));
        }
        catch {
    ...
    const wallItems = status.achievements
        ...
        if (wallSearch.trim() !== '') {
            ...
            if (!a.name.zh.toLowerCase().includes(q) && !a.name.en.toLowerCase().includes(q) && !a.id.toLowerCase().includes(q))
                return false;
        }
        if (wallRarity !== 'all' && a.rarity !== wallRarity)
            return false;
        if (wallStatus === 'unlocked' && !a.unlocked)
            return false;
        if (wallStatus === 'locked' && a.unlocked)
            return false;
        return true;
    });
    ...
    return _jsxs("section", { ref: cardRef, style: {
            ...cardStyle,
            ...positionStyle,
            ...
            ...(settings.compact ? {
                ...
                '--dq-hero-mb': '8px',
        ...
        }, "data-devquest": true, onPointerDown: onCardPointerDown, onPointerMove: onCardPointerMove, onPointerUp: onCardPointerUp, onPointerCancel: onCardPointerCancel, children: [_jsxs("header", { style: cardHeaderStyle, children: [_jsx("span", { style: { color: TONE.accent, display: 'inline-flex' }, children: _jsx(SwordIcon, { size: 20 }) }), _jsx("strong", { style: cardTitleStyle, children: "DevQuest" }), status.version !== undefined && status.version !== '' && (_jsx("span", { style: versionLabelStyle, title: t('dq.version'), children: status.version })), _jsx("button", { type: "button", onClick: expandAll, "aria-label": t('dq.expandAll'), title: t('dq.expandAll'), style: iconButtonStyle, children: "\u2922" }), _jsx("button", { type: "button", onClick: collapseAll, "aria-label": t('dq.collapseAll'), title: t('dq.collapseAll'), style: iconButtonStyle, children: "\u2921" }), _jsx("button", { type: "button", onClick: () => actions.setOpen(false), "aria-label": t('dq.close'), style: iconButtonStyle, children: _jsx(CloseIcon, {}) })] }), _jsxs("div", { style: cardBodyStyle, children: [_jsxs("div", { style: heroStyle, children: [_jsxs("div", { style: { position: 'relative' }, children: [_jsx(LevelRing, { status: status }), _jsxs("div", { style: levelBadgeStyle, children: [_jsxs("span", { style: levelNumStyle, children: ["Lv.", status.level] }), _jsx("span", { style: { ...levelSubStyle, ...titleToneStyle(status.level) }, children: status.title.zh })] })] }), _jsxs("div", { style: { flex: 1, minWidth: 0 }, children: [_jsxs("div", { style: titleRowStyle, children: [_jsx("span", { style: { ...titleTextStyle, ...titleToneStyle(status.level) }, children: status.title.zh }), (status.shop?.badges ?? []).map(badgeId => {
                        ...
                        return (_jsxs("div", { style: dailyGoalCardStyle, children: [_jsxs("div", { style: dailyGoalRowStyle, children: [_jsxs("span", { style: dailyGoalLabelStyle, children: ["\uD83C\uDFAF ", t('dq.dailyGoal')] }), _jsx("span", { style: dailyGoalNumStyle, children: t('dq.dailyGoalProgress', { xp: formatNumber(g.todayXp), goal: formatNumber(g.goal) }) })] }), _jsx("div", { style: dailyGoalTrackStyle, children: _jsx("div", { style: { ...dailyGoalFillStyle, width: `${pct}%`, ...(reached ? questFillDoneStyle : {}) } }) }), g.claimed
                    ...
                    })(), panelMsg !== null && (_jsxs("div", { style: panelMsgStyle(panelMsg.ok), role: "status", children: [panelMsg.ok ? '✅ ' : '⚠️ ', panelMsg.text] })), _jsx(SectionCard, { id: "ritual", title: `🌅 ${t('dq.ritual')}`, collapsed: isCollapsed('ritual'), onToggle: () => toggleSection('ritual'), children: _jsxs("div", { style: ritualStyle, children: [_jsx("div", { style: ritualGreetingStyle, children: t('dq.ritualGreeting', { level: status.level }) }), questReminderMsg !== null && _jsxs("div", { style: ritualReminderStyle, children: ["\u23F0 ", questReminderMsg] }), (() => {
                                ...
                                : _jsxs("button", { type: "button", onClick: () => void claimChest(), disabled: claiming, style: chestButtonStyle, children: ["\uD83C\uDF81 ", claiming ? t('dq.chestClaiming') : t('dq.chestReady', { xp: 50 })] }))] }), _jsx(SectionCard, { id: "weekly", title: `🗓️ ${t('dq.weekly')}`, right: _jsx("span", { style: updatedStyle, children: t('dq.weeklyWeek', { week: status.weekly?.week ?? '' }) }), collapsed: isCollapsed('weekly'), onToggle: () => toggleSection('weekly'), children: status.weekly !== undefined && (_jsxs(_Fragment, { children: [status.weekly.quests.map(q => {
                                    const pct = Math.min(100, Math.round((Math.min(q.progress, q.goal) / Math.max(q.goal, 1)) * 100));
                            ...
                            : _jsxs("span", { style: titleHeadCurrentStyle, children: [t('dq.titleFollowLevel'), " \u00B7 ", status.title.zh] }), collapsed: isCollapsed('titles'), onToggle: () => toggleSection('titles'), children: [_jsxs("div", { style: titleCurrentRowStyle, children: [_jsx("span", { style: { fontSize: 'calc(15px * var(--dq-fsz, 1))' }, children: status.titles?.current?.icon ?? '🎖️' }), _jsx("span", { style: titleCurrentNameStyle, children: status.titles?.current !== null
                                            ...
                                            ...(status.titles?.current?.id === ti.id ? titleItemActiveStyle : {}),
                                                    ...
                                                    if (f !== undefined)
                                                        void importSave(f);
                                                    e.target.value = '';
                                                } })] })] })] }), _jsx(SectionCard, { id: "pokedex", title: `📖 ${t('dq.pokedex')}`, right: _jsxs("span", { style: updatedStyle, children: [t('dq.pokedexOverall', { pct: Math.round(((unlocked.length / Math.max(status.achievements.length, 1)) + ((status.shop?.themes ?? []).length / Math.max(status.shop?.items.filter(i => i.kind === 'theme').length, 1)) + ((status.titles?.items ?? []).filter(t => t.unlocked).length / Math.max(status.titles?.items?.length ?? 1, 1))) / 3 * 100) }), "%"] }), collapsed: isCollapsed('pokedex'), onToggle: () => toggleSection('pokedex'), children: _jsxs("div", { style: pokedexGridStyle, children: [_jsxs("div", { style: pokedexItemStyle, children: [_jsx("span", { style: pokedexIconStyle, children: "\uD83C\uDFC6" }), _jsx("span", { style: pokedexNameStyle, children: t('dq.pokedexAch') }), _jsx("div", { style: pokedexTrackStyle, children: _jsx("div", { style: { ...pokedexFillStyle, width: `${Math.round(unlocked.length / Math.max(status.achievements.length, 1) * 100)}%` } }) }), _jsxs("span", { style: pokedexNumStyle, children: [unlocked.length, "/", status.achievements.length] })] }), _jsxs("div", { style: pokedexItemStyle, children: [_jsx("span", { style: pokedexIconStyle, children: "\uD83C\uDFA8" }), _jsx("span", { style: pokedexNameStyle, children: t('dq.pokedexSkin') }), _jsx("div", { style: pokedexTrackStyle, children: _jsx("div", { style: { ...pokedexFillStyle, width: `${Math.round((status.shop?.themes ?? []).length / Math.max(status.shop?.items.filter(i => i.kind === 'theme').length, 1) * 100)}%` } }) }), _jsxs("span", { style: pokedexNumStyle, children: [(status.shop?.themes ?? []).length, "/", status.shop?.items.filter(i => i.kind === 'theme').length ?? 0] })] }), _jsxs("div", { style: pokedexItemStyle, children: [_jsx("span", { style: pokedexIconStyle, children: "\uD83C\uDFF7\uFE0F" }), _jsx("span", { style: pokedexNameStyle, children: t('dq.pokedexTitle') }), _jsx("div", { style: pokedexTrackStyle, children: _jsx("div", { style: { ...pokedexFillStyle, width: `${Math.round((status.titles?.items ?? []).filter(t => t.unlocked).length / Math.max(status.titles?.items?.length ?? 1, 1) * 100)}%` } }) }), _jsxs("span", { style: pokedexNumStyle, children: [(status.titles?.items ?? []).filter(t => t.unlocked).length, "/", status.titles?.items?.length ?? 0] })] })] }) }), _jsx(SectionCard, { id: "recent", title: t('dq.recent'), right: _jsxs("span", { style: updatedStyle, children: [t('dq.updated'), " ", updatedLabel(state.refreshedAt)] }), collapsed: isCollapsed('recent'), onToggle: () => toggleSection('recent'), children: recent.length === 0
...
function DevQuestToast(props) {
    ...
    return _jsxs("div", { style: { ...toastStyle, ...rarityToastStyle(def.rarity) }, role: "status", children: [_jsx("div", { style: { fontSize: 'calc(18px * var(--dq-fsz, 1))' }, children: def.icon }), _jsxs("div", { style: { minWidth: 0 }, children: [_jsxs("div", { style: { ...toastTitleStyle, color: RARITY_COLOR[def.rarity] ?? TONE.gold }, children: [t('dq.unlocked'), " ", _jsxs("span", { style: { fontSize: 'calc(9px * var(--dq-fsz, 1))', opacity: 0.8 }, children: ["\u00B7 ", t(`dq.rarity.${def.rarity}`)] })] }), _jsxs("div", { style: toastNameStyle, children: [def.name.zh, " ", _jsx("em", { style: itemEnStyle, children: def.name.en })] }), _jsxs("div", { style: toastDescStyle, children: [def.description.zh, " \u00B7 +", def.xp, " XP"] })] }), _jsx("button", { type: "button", onClick: () => actions.dismissToast(toast.id), "aria-label": t('dq.close'), style: toastCloseStyle, children: "\u00D7" })] });
}
// ---------------------------------------------------------------------------
// 入口组件
// ---------------------------------------------------------------------------
...
export function DevQuestOverlay(props) {
    ...
    return _jsxs(_Fragment, { children: [state.open && (_jsx(DevQuestPanelCard, { useStore: useStore, actions: actions, t: t, refresh: refresh })), state.toasts.length > 0 && state.status !== null && (_jsx("div", { style: toastStackStyle, children: (() => {
                    ...
                    const visible = filter === 'off' ? [] : state.toasts.filter(toast => {
                        if (filter === 'all')
                            return true;
                        if (toast.kind !== 'achievement')
                            return true; // 回合结算 toast 不受稀有度过滤
...
const cardTitleStyle = { fontSize: 'calc(14px * var(--dq-fsz, 1))', color: TONE.text, letterSpacing: 0.2 };
...
const heroStyle = { display: 'flex', gap: 12, alignItems: 'center', marginBottom: 'var(--dq-hero-mb, 12px)' };
...
const levelNumStyle = { fontSize: 'calc(15px * var(--dq-fsz, 1))', fontWeight: 700, color: TONE.text, lineHeight: 1.1 };
const levelSubStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.muted };
const titleRowStyle = { display: 'flex', alignItems: 'baseline', gap: 8 };
const titleTextStyle = { fontSize: 'calc(13px * var(--dq-fsz, 1))', fontWeight: 600, color: TONE.text };
const seasonStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.quiet };
const xpTrackStyle = {
    height: 7,
    borderRadius: 4,
...
const xpTextStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.muted };
const metaRowStyle = { display: 'flex', flexWrap: 'wrap', gap: '4px 10px', marginTop: 6 };
const metaStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.quiet, background: TONE.row, padding: '2px 6px', borderRadius: 5 };
const comboStyle = {
    fontSize: 'calc(10px * var(--dq-fsz, 1))',
    fontWeight: 700,
    color: TONE.gold,
...
const questLabelStyle = { fontSize: 'calc(11px * var(--dq-fsz, 1))', color: TONE.text };
const questRewardStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', fontWeight: 600, color: TONE.gold };
...
const updatedStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.quiet };
...
const itemNameStyle = { fontSize: 'calc(12px * var(--dq-fsz, 1))', color: TONE.text };
const itemEnStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.quiet, fontStyle: 'normal', marginLeft: 4 };
const itemTimeStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.quiet };
const linkButtonStyle = { border: 'none', background: 'transparent', color: TONE.muted, cursor: 'pointer', fontSize: 'calc(11px * var(--dq-fsz, 1))', padding: '0 4px' };
const tabsStyle = { display: 'flex', gap: 4, flexWrap: 'wrap' };
/** 成就墙筛选行：搜索框 + 稀有度/状态下拉。 */
const wallFilterRowStyle = { display: 'flex', gap: 5, marginBottom: 6, alignItems: 'center' };
...
const wallXpStyle = { fontSize: 'calc(8px * var(--dq-fsz, 1))', color: TONE.quiet };
...
const milestoneIconStyle = { fontSize: 'calc(16px * var(--dq-fsz, 1))', lineHeight: 1 };
const milestoneTopStyle = { display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 8 };
const milestoneNameStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.text, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' };
const milestoneNumStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.muted, fontVariantNumeric: 'tabular-nums' };
...
const tooltipProgressLabelStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.quiet, textTransform: 'uppercase', letterSpacing: 0.3 };
const tooltipProgressNumStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.muted, fontVariantNumeric: 'tabular-nums' };
...
// ---- P1/P2 样式 ----
/** 已购称号徽章（称号旁小图标）。 */
const titleBadgeStyle = { fontSize: 'calc(13px * var(--dq-fsz, 1))', lineHeight: 1, marginLeft: -2 };
/** 等级持续天数。 */
const levelSinceStyle = { display: 'block', fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.quiet, marginTop: 1 };
/** 赛季冲刺条。 */
const sprintRowStyle = { display: 'flex', alignItems: 'center', gap: 6, marginTop: 5 };
const sprintLabelStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.quiet, whiteSpace: 'nowrap' };
...
const sprintDaysStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.muted, whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' };
/** v1.1 连续活跃行。 */
const streakRowStyle = { display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 };
...
const streakNextStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.quiet };
const boostStockStyle = { marginLeft: 'auto', fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.gold, whiteSpace: 'nowrap' };
/** v1.1 赛季通行证行。 */
const passRowStyle = { display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 };
/** v1.1 每日开工仪式。 */
const ritualStyle = { display: 'flex', flexDirection: 'column', gap: 5 };
const ritualGreetingStyle = { fontSize: 'calc(12px * var(--dq-fsz, 1))', fontWeight: 700, color: 'var(--dsw-alias-label-primary, #1a2230)' };
const ritualSummaryStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.muted };
...
const ritualGoalsStyle = { display: 'flex', flexWrap: 'wrap', gap: 4 };
...
/** v1.1 收藏图鉴总览。 */
const pokedexGridStyle = { display: 'flex', flexDirection: 'column', gap: 6 };
const pokedexItemStyle = { display: 'flex', alignItems: 'center', gap: 7 };
const pokedexIconStyle = { fontSize: 'calc(14px * var(--dq-fsz, 1))', width: 18, textAlign: 'center' };
const pokedexNameStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.text, width: 52, flexShrink: 0 };
...
const pokedexNumStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.quiet, width: 34, textAlign: 'right', fontVariantNumeric: 'tabular-nums' };
/** v1.2.0 设置区。 */
const settingsRowStyle = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '3px 0' };
const settingsLabelStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.text };
const settingsControlStyle = { display: 'flex', alignItems: 'center', gap: 6 };
const settingsValueStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.gold, minWidth: 36, textAlign: 'center', fontVariantNumeric: 'tabular-nums' };
...
const passTrackStyle = { display: 'flex', gap: 3, flex: 1 };
...
const shopBalanceStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.gold, fontWeight: 600, fontVariantNumeric: 'tabular-nums' };
const shopStockStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.muted };
...
const shopItemNameStyle = { flex: 1, fontSize: 'calc(11px * var(--dq-fsz, 1))', color: TONE.text, fontWeight: 600 };
const shopItemPriceStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.gold, fontWeight: 700, fontVariantNumeric: 'tabular-nums' };
const shopItemDescStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.muted, marginTop: 3, lineHeight: 1.4 };
const shopOwnedStyle = { marginTop: 5, fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.green };
...
const skinSwatchRowStyle = { display: 'flex', alignItems: 'center', gap: 4, marginTop: 4 };
...
const dailyGoalRowStyle = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 4 };
const dailyGoalLabelStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', fontWeight: 600, color: TONE.text };
const dailyGoalNumStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.gold, fontVariantNumeric: 'tabular-nums' };
const dailyGoalTrackStyle = { height: 5, borderRadius: 3, background: TONE.row, overflow: 'hidden' };
const dailyGoalFillStyle = { height: '100%', borderRadius: 3, background: 'linear-gradient(90deg, var(--dsw-alias-state-warn-primary, #f6c652), var(--dsw-alias-state-success-primary, #78dda0))' };
const dailyGoalDoneStyle = { marginTop: 4, fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.green };
...
const bossHeadRowStyle = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 4 };
const bossNameStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', fontWeight: 700, color: TONE.red };
const bossHpStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.muted, fontVariantNumeric: 'tabular-nums' };
const bossTrackStyle = { height: 6, borderRadius: 3, background: TONE.row, overflow: 'hidden' };
const bossFillStyle = { height: '100%', borderRadius: 3, background: 'linear-gradient(90deg, var(--dsw-alias-state-error-primary, #ff8592), var(--dsw-alias-state-warn-primary, #f6c652))' };
const bossHintStyle = { marginTop: 4, fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.quiet };
...
const classBadgeNameStyle = { flex: 1, fontSize: 'calc(11px * var(--dq-fsz, 1))', fontWeight: 600, color: TONE.text };
const classBadgeLabelStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.accent, textTransform: 'uppercase', letterSpacing: 0.3 };
...
const seasonSummaryHeadStyle = { fontSize: 'calc(11px * var(--dq-fsz, 1))', fontWeight: 700, color: TONE.gold };
const seasonSummaryMetaStyle = { marginTop: 3, fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.text };
const seasonSummaryRewardStyle = { marginTop: 3, fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.green };
/** 新手任务链。 */
const tutorialRowStyle = { display: 'flex', alignItems: 'center', gap: 7, padding: '3px 0' };
const tutorialNameStyle = { flex: 1, fontSize: 'calc(11px * var(--dq-fsz, 1))', color: TONE.text };
const tutorialXpStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.gold };
const tutorialTitleStyle = { marginTop: 4, fontSize: 'calc(11px * var(--dq-fsz, 1))', color: TONE.gold, fontWeight: 700 };
/** 成长周报。 */
...
const reportBarDateStyle = { fontSize: 'calc(8px * var(--dq-fsz, 1))', color: TONE.quiet, overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100%' };
const reportLegendStyle = { marginTop: 4, fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.quiet, textAlign: 'center' };
...
const celebrationTitleStyle = { fontSize: 'calc(20px * var(--dq-fsz, 1))', fontWeight: 800, color: TONE.gold, letterSpacing: 1 };
const celebrationLevelStyle = { fontSize: 'calc(16px * var(--dq-fsz, 1))', fontWeight: 600, color: TONE.text };
const celebrationStatsStyle = { fontSize: 'calc(12px * var(--dq-fsz, 1))', color: TONE.muted };
/** 活跃日历。 */
...
const calendarLegendLabelStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.quiet };
...
const statsChipStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.text, background: TONE.row, padding: '4px 8px', borderRadius: 7 };
const statsSubTitleStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', fontWeight: 600, color: TONE.muted, marginTop: 2 };
...
const toolRankNumStyle = { width: 16, fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.quiet, fontWeight: 700 };
const toolRankNameStyle = { flex: 1, fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' };
const toolRankCountStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.gold, fontVariantNumeric: 'tabular-nums' };
/** 荣誉墙。 */
const recordRowStyle = { display: 'flex', flexWrap: 'wrap', gap: 5 };
const recordChipStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.gold, background: 'color-mix(in srgb, var(--dsw-alias-state-warn-primary, #f6c652) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--dsw-alias-state-warn-primary, #f6c652) 30%, transparent)', padding: '3px 7px', borderRadius: 6 };
/** 下一称号预览行 + 幸运抽奖。 */
const nextTitleRowStyle = { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 12 };
const nextTitleStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.muted };
...
const luckyMsgStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.gold, marginTop: 2 };
/** 分类收藏行。 */
const collRowStyle = { display: 'flex', alignItems: 'center', gap: 7, padding: '3px 0' };
const collNameStyle = { flex: 1, fontSize: 'calc(11px * var(--dq-fsz, 1))', color: TONE.text };
const collProgressStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.muted, fontVariantNumeric: 'tabular-nums' };
const collRewardStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.quiet };
...
const weeklyQuestLabelStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.text };
const weeklyQuestRewardStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', fontWeight: 600, color: TONE.gold };
...
const weeklyBonusClaimedStyle = { marginTop: 4, fontSize: 'calc(10px * var(--dq-fsz, 1))', color: TONE.quiet };
/** 多称号。 */
const titleCurrentRowStyle = { display: 'flex', alignItems: 'center', gap: 8 };
const titleCurrentNameStyle = { flex: 1, fontSize: 'calc(12px * var(--dq-fsz, 1))', color: TONE.text, fontWeight: 600 };
...
const titleItemActiveMarkStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.gold, fontWeight: 600 };
const titleItemLockedMarkStyle = { fontSize: 'calc(9px * var(--dq-fsz, 1))', color: TONE.quiet };
/** 存档管理。 */
...
/** 周报回合数标注。 */
const reportBarTurnStyle = { fontSize: 'calc(8px * var(--dq-fsz, 1))', color: TONE.quiet, fontVariantNumeric: 'tabular-nums' };
...
const tooltipNameStyle = { fontSize: 'calc(12px * var(--dq-fsz, 1))', fontWeight: 600, color: TONE.text };
const tooltipStatusStyle = { display: 'flex', alignItems: 'center', gap: 8, marginTop: 2 };
const tooltipXpStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', fontWeight: 700, color: TONE.gold };
const tooltipDescStyle = { fontSize: 'calc(11px * var(--dq-fsz, 1))', color: TONE.muted, marginTop: 6, lineHeight: 1.5 };
const emptyStyle = { fontSize: 'calc(11px * var(--dq-fsz, 1))', color: TONE.quiet, padding: '8px 0' };
...
const toastTitleStyle = { fontSize: 'calc(10px * var(--dq-fsz, 1))', fontWeight: 700, color: TONE.gold, textTransform: 'uppercase', letterSpacing: 0.4 };
const toastNameStyle = { fontSize: 'calc(13px * var(--dq-fsz, 1))', fontWeight: 600, color: TONE.text, marginTop: 2 };
const toastDescStyle = { fontSize: 'calc(11px * var(--dq-fsz, 1))', color: TONE.muted, marginTop: 2 };
...
const footerLabelStyle = { fontWeight: 600, fontSize: 'calc(12px * var(--dq-fsz, 1))' };
