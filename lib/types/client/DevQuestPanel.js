...
import { TONE, themeVars } from "./panel/theme.js";
import { SwordIcon, CloseIcon } from "./panel/icons.js";
import { STATUS_API, POLL_MS, levelPercent, apiErrorOf, playSfx, dayKeyLocal, RARITY_COLOR, rarityToastStyle, PANEL_POS_KEY, loadPanelPos, REMINDER_KEY, loadCollapsed, saveCollapsed, loadSettings, saveSettings, fetchUiSettings, rarityWeight, clampPanelPos } from "./panel/util.js";
import { HeroSection, SeasonSummaryCard, DailyGoalCard, RitualSection, LuckyRow, DailySection, WeeklySection, ShopSection, SkinsSection, TutorialSection, TitlesSection, CollectionsSection, PokedexSection, RecentSection, WallSection, ReportSection, CalendarSection, StatsSection, SettingsSection, AdventureSection, ChainSection, GhostSection, RelicsSection } from "./panel/sections.js";
import { cardStyle, cardHeaderStyle, cardDraggingStyle, cardTitleStyle, versionLabelStyle, iconButtonStyle, cardBodyStyle, itemEnStyle, panelMsgStyle, celebrationOverlayStyle, celebrationInnerStyle, celebrationTitleStyle, celebrationLevelStyle, celebrationStatsStyle, emptyStyle, toastStackStyle, toastStyle, toastTitleStyle, toastNameStyle, toastDescStyle, toastCloseStyle, footerActionStyle, railActionStyle, footerActionActiveStyle, footerLabelStyle, levelChipStyle } from "./panel/styles.js";
// ---------------------------------------------------------------------------
// 面板卡片
// ---------------------------------------------------------------------------
/**
...
export function DevQuestPanelCard(props) {
    ...
    const [eventResolving, setEventResolving] = useState(null);
    /** 结算事件卡抉择（choice 型：选项 0/1）。 */
    const resolveEventF = useCallback(async (eventId, option) => {
        if (eventResolving !== null)
            return;
    ...
    /** 领取史诗任务链终章奖励。 */
    const claimChainF = useCallback(async () => {
        if (chainClaiming)
            return;
    ...
    /** 领取幽灵竞速奖励（击败上周的自己）。 */
    const claimGhostF = useCallback(async () => {
        if (ghostClaiming)
            return;
    ...
    return _jsxs("section", { ref: cardRef, style: {
        ...
        }, "data-devquest": true, onPointerDown: onCardPointerDown, onPointerMove: onCardPointerMove, onPointerUp: onCardPointerUp, onPointerCancel: onCardPointerCancel, children: [_jsxs("header", { style: cardHeaderStyle, children: [_jsx("span", { style: { color: TONE.accent, display: 'inline-flex' }, children: _jsx(SwordIcon, { size: 20 }) }), _jsx("strong", { style: cardTitleStyle, children: "DevQuest" }), status.version !== undefined && status.version !== '' && (_jsx("span", { style: versionLabelStyle, title: t('dq.version'), children: status.version })), _jsx("button", { type: "button", onClick: expandAll, "aria-label": t('dq.expandAll'), title: t('dq.expandAll'), style: iconButtonStyle, children: "\u2922" }), _jsx("button", { type: "button", onClick: collapseAll, "aria-label": t('dq.collapseAll'), title: t('dq.collapseAll'), style: iconButtonStyle, children: "\u2921" }), _jsx("button", { type: "button", onClick: () => actions.setOpen(false), "aria-label": t('dq.close'), style: iconButtonStyle, children: _jsx(CloseIcon, {}) })] }), _jsxs("div", { style: cardBodyStyle, children: [_jsx(HeroSection, { status: status, t: t, c: c, percent: percent, refresh: refresh, claimPassTier: claimPassTier }), _jsx(SeasonSummaryCard, { status: status, t: t }), _jsx(DailyGoalCard, { status: status, t: t, claimDailyGoalF: claimDailyGoalF }), panelMsg !== null && (_jsxs("div", { style: panelMsgStyle(panelMsg.ok), role: "status", children: [panelMsg.ok ? '✅ ' : '⚠️ ', panelMsg.text] })), _jsx(RitualSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, questReminderMsg: questReminderMsg }), _jsx(LuckyRow, { status: status, t: t, claimingLucky: claimingLucky, luckyMsg: luckyMsg, claimLuckyDraw: claimLuckyDraw }), _jsx(DailySection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, claiming: claiming, claimChest: claimChest }), _jsx(WeeklySection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, weeklyClaiming: weeklyClaiming, claimBossF: claimBossF, claimWeekly: claimWeekly }), _jsx(ShopSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, buying: buying, confirmBuyId: confirmBuyId, buy: buy, rerolling: rerolling, rerollQuests: rerollQuests, useQuestSkipCard: useQuestSkipCard }), _jsx(SkinsSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, buying: buying, confirmBuyId: confirmBuyId, buy: buy, activateTheme: activateTheme }), _jsx(TutorialSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t }), _jsx(TitlesSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, sharing: sharing, shareCard: shareCard, shareSeason: shareSeason, switchTitle: switchTitle }), _jsx(CollectionsSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, importing: importing, exportSave: exportSave, importSave: importSave }), _jsx(PokedexSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, unlocked: unlocked }), _jsx(RecentSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, state: state, recent: recent }), _jsx(WallSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, category: category, setCategory: setCategory, wallSearch: wallSearch, setWallSearch: setWallSearch, wallRarity: wallRarity, setWallRarity: setWallRarity, wallStatus: wallStatus, setWallStatus: setWallStatus, hover: hover, setHover: setHover, wallItems: wallItems, milestone: milestone, unlocked: unlocked }), _jsx(ReportSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t }), _jsx(CalendarSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t }), _jsx(StatsSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, c: c }), _jsx(SettingsSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, settings: settings, updateSettings: updateSettings, setGoalF: setGoalF }), _jsx(AdventureSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, onResolve: (id, opt) => void resolveEventF(id, opt), resolving: eventResolving }), _jsx(ChainSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, onClaim: claimChainF, claiming: chainClaiming }), _jsx(GhostSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t, onClaim: claimGhostF, claiming: ghostClaiming }), _jsx(RelicsSection, { collapsedMap: collapsed, toggle: toggleSection, status: status, t: t })] })] });
}
// ---------------------------------------------------------------------------
// 成就 toast
// ---------------------------------------------------------------------------
...
export function DevQuestOverlay(props) {
    ...
    useEffect(() => {
        const onUiSettings = () => setUiSettingsVersion(v => v + 1);
        window.addEventListener('devquest:ui-settings', onUiSettings);
        return () => window.removeEventListener('devquest:ui-settings', onUiSettings);
    }, []);
    ...
    useEffect(() => {
        if (state.status === null || state.toasts.length === 0)
            return;
        ...
        for (const toast of state.toasts) {
            ...
            if (toast.kind === 'achievement') {
                ...
                if (def !== undefined && rarityWeight(def.rarity) < rarityWeight('rare'))
                    actions.dismissToast(toast.id);
            }
        }
