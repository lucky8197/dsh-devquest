    ...
    const claimPassTier = useCallback(async (tierId) => {
        try {
            ...
            if (data.status !== null && data.status !== undefined)
                actions.setStatus(data.status);
            if (data.ok)
                setShopMsg({ ok: true, text: t('dq.passClaimed', { xp: data.gained }) });
        }
        catch {
            // 静默失败
        }
    ...
    const useQuestSkipCard = useCallback(async () => {
        try {
            ...
            if (data.status !== null && data.status !== undefined)
                actions.setStatus(data.status);
            if (data.ok)
                setShopMsg({ ok: true, text: t('dq.skipUsed') });
        }
        catch {
            // 静默失败
        }
