/**
 * DevQuest locale dictionaries（中英双语）。
 * 面板组件通过框架注入的 `t` seat 读取，key 域即本字典。
 */
export declare const NS = "devquest";
declare const zh: {
    readonly 'dq.open': "DevQuest 进度";
    readonly 'dq.level': "Lv.{level}";
    readonly 'dq.xpToNext': "{xp} / {next} XP";
    readonly 'dq.season': "赛季 {season}";
    readonly 'dq.recent': "最近成就";
    readonly 'dq.wall': "成就墙";
    readonly 'dq.wallCount': "{n}/{m}";
    readonly 'dq.wallSearch': "搜索成就…";
    readonly 'dq.wallNoMatch': "没有匹配的成就";
    readonly 'dq.wallRarityAll': "全部稀有度";
    readonly 'dq.wallStatusAll': "全部状态";
    readonly 'dq.wallStatusUnlocked': "已解锁";
    readonly 'dq.wallStatusLocked': "未解锁";
    readonly 'dq.refresh': "刷新";
    readonly 'dq.loading': "加载中…";
    readonly 'dq.error': "加载失败";
    readonly 'dq.close': "关闭";
    readonly 'dq.empty': "暂无数据，完成一个回合试试";
    readonly 'dq.unlocked': "成就解锁！";
    readonly 'dq.cat.journey': "旅程";
    readonly 'dq.cat.crafting': "锻造";
    readonly 'dq.cat.quest': "使命";
    readonly 'dq.cat.time': "时光";
    readonly 'dq.cat.legend': "传奇";
    readonly 'dq.cat.egg': "彩蛋";
    readonly 'dq.updated': "更新于";
    readonly 'dq.turns': "{n} 回合";
    readonly 'dq.toolCalls': "{n} 次工具调用";
    readonly 'dq.todos': "{n} 个待办";
    readonly 'dq.tokens': "{n} tokens";
    readonly 'dq.consecutive': "连击 {n}";
    readonly 'dq.combo': "连击 {n}";
    readonly 'dq.streak': "活跃 {n} 天";
    readonly 'dq.daily': "每日任务";
    readonly 'dq.earned': "已解锁";
    readonly 'dq.notEarned': "未解锁";
    readonly 'dq.hiddenHint': "隐藏成就，解锁后可见";
    readonly 'dq.nextMilestone': "最近的里程碑 · {name}";
    readonly 'dq.progress': "进度";
    readonly 'dq.turnDone': "回合结算";
    readonly 'dq.levelUp': "升级！Lv.{level}";
    readonly 'dq.levelUpTo': "新称号 · {title}";
    readonly 'dq.turnStats': "完成 {turns} 个回合";
    readonly 'dq.chestReady': "领取全清宝箱 +{xp} XP";
    readonly 'dq.chestClaiming': "领取中…";
    readonly 'dq.chestClaimed': "全清宝箱已领取";
    readonly 'dq.tutorial': "新手任务";
    readonly 'dq.tutorialDone': "已完成全部新手任务";
    readonly 'dq.tutorialTitle': "专属称号 · {title}";
    readonly 'dq.tutorialStepDone': "{n}/{m} 步";
    readonly 'dq.version': "插件版本号";
    readonly 'dq.expandAll': "全部展开";
    readonly 'dq.collapseAll': "全部折叠";
    readonly 'dq.streakBest': "历史最高连续 {best} 天";
    readonly 'dq.streakNext': "下一档 +{xp} XP";
    readonly 'dq.pass': "赛季通行证";
    readonly 'dq.passClaimed': "通行证奖励 +{xp} XP";
    readonly 'dq.ritual': "今日开工";
    readonly 'dq.ritualGreeting': "Lv.{level}，今天也要加油！";
    readonly 'dq.ritualYesterday': "昨天 +{xp} XP · {turns} 回合";
    readonly 'dq.ritualFirst': "今天开始你的第一条冒险吧";
    readonly 'dq.pokedex': "收藏图鉴";
    readonly 'dq.pokedexOverall': "总完成度 {pct}%";
    readonly 'dq.pokedexAch': "成就";
    readonly 'dq.pokedexSkin': "皮肤";
    readonly 'dq.pokedexTitle': "称号";
    readonly 'dq.shopSkip': "跳过任务";
    readonly 'dq.skipUsed': "已跳过 1 个任务";
    readonly 'dq.reminder': "今天还有 {n} 个每日任务没完成，趁现在做完吧！";
    readonly 'dq.shareSeason': "赛季报告";
    readonly 'dq.settings': "设置";
    readonly 'dq.settingsFont': "字号";
    readonly 'dq.settingsCompact': "紧凑模式";
    readonly 'dq.settingsToast': "通知过滤";
    readonly 'dq.settingsToastAll': "全部通知";
    readonly 'dq.settingsToastRare': "仅稀有以上";
    readonly 'dq.settingsToastOff': "关闭通知";
    readonly 'dq.dailyGoal': "今日目标";
    readonly 'dq.dailyGoalSet': "设定今日 XP 目标";
    readonly 'dq.dailyGoalOff': "关闭";
    readonly 'dq.dailyGoalClaim': "🎯 领取 +{xp} XP";
    readonly 'dq.dailyGoalClaimed': "今日目标已达成 ✓";
    readonly 'dq.dailyGoalProgress': "{xp} / {goal} XP";
    readonly 'dq.dailyGoalHint': "今日目标 · 达成领奖";
    readonly 'dq.classLabel': "职业";
    readonly 'dq.classNone': "未定（多用工具解锁职业）";
    readonly 'dq.boss': "每周 BOSS";
    readonly 'dq.bossHp': "{damage} / {hp}";
    readonly 'dq.bossDefeat': "⚔️ 击败 {name}！领取 +{n} 赛季货币";
    readonly 'dq.bossClaimed': "BOSS 掉落已领取";
    readonly 'dq.bossProgress': "完成全部周挑战击败 BOSS";
    readonly 'dq.seasonSummary': "上赛季结算";
    readonly 'dq.seasonSummaryTitle': "赛季 {season} · Lv.{level}";
    readonly 'dq.seasonSummaryMeta': "最高连击 {combo} · 赛季 XP {xp} · 成就 {n} 枚";
    readonly 'dq.seasonSummaryReward': "赛季纪念奖励 +200 XP 已入账";
    readonly 'dq.sound': "音效提示";
    readonly 'dq.notify': "桌面通知";
    readonly 'dq.on': "开";
    readonly 'dq.off': "关";
    readonly 'dq.levelSince': "本等级已 {days} 天";
    readonly 'dq.seasonSprint': "赛季冲刺";
    readonly 'dq.seasonDaysLeft': "剩 {days} 天";
    readonly 'dq.seasonGoal': "赛季输出 {tokens} / {goal}";
    readonly 'dq.report': "成长周报";
    readonly 'dq.report7d': "最近 7 天";
    readonly 'dq.reportXp': "{xp} XP";
    readonly 'dq.shop': "商店";
    readonly 'dq.shopBalance': "赛季货币 {balance}";
    readonly 'dq.shopBuy': "购买";
    readonly 'dq.shopConfirm': "确认购买？";
    readonly 'dq.shopOwned': "已拥有";
    readonly 'dq.themeActive': "使用中";
    readonly 'dq.themeUse': "使用";
    readonly 'dq.themeUsed': "已切换主题";
    readonly 'dq.skins': "主题皮肤";
    readonly 'dq.skinDefault': "默认主题";
    readonly 'dq.shopNoBalance': "赛季货币不足";
    readonly 'dq.shopBought': "购买成功";
    readonly 'dq.shopReroll': "重掷任务";
    readonly 'dq.shopShields': "保险 ×{n}";
    readonly 'dq.shopRerolls': "重掷 ×{n}";
    readonly 'dq.hiddenNear': "接近解锁的隐藏成就";
    readonly 'dq.rarity.common': "普通";
    readonly 'dq.rarity.rare': "稀有";
    readonly 'dq.rarity.epic': "史诗";
    readonly 'dq.rarity.legendary': "传说";
    readonly 'dq.collections': "分类收藏";
    readonly 'dq.collectionDone': "已集齐";
    readonly 'dq.collectionProgress': "{n}/{m}";
    readonly 'dq.collectionReward': "集齐奖励 +{xp} XP";
    readonly 'dq.lucky': "每日幸运";
    readonly 'dq.luckyDraw': "🎁 今日幸运抽奖";
    readonly 'dq.luckyClaimed': "今日已抽";
    readonly 'dq.luckyResult': "抽到：{label}";
    readonly 'dq.nextTitle': "距 {name}（Lv.{level}）还差 {xp} XP";
    readonly 'dq.export': "导出存档";
    readonly 'dq.import': "导入存档";
    readonly 'dq.exported': "存档已导出";
    readonly 'dq.imported': "存档已导入";
    readonly 'dq.importFailed': "导入失败：存档格式无效";
    readonly 'dq.opFailed': "操作失败";
    readonly 'dq.titleSwitched': "已切换称号";
    readonly 'dq.rerolled': "任务已重掷";
    readonly 'dq.weeklyClaimed': "全清周奖励 +{xp} XP";
    readonly 'dq.weekly': "每周挑战";
    readonly 'dq.weeklyWeek': "{week} 周";
    readonly 'dq.weeklyBonus': "🎁 领取全清周奖励 +{xp} XP";
    readonly 'dq.weeklyBonusClaimed': "全清周奖励已领取";
    readonly 'dq.titles': "称号";
    readonly 'dq.titleLevel': "等级称号";
    readonly 'dq.titleFollowLevel': "跟随等级";
    readonly 'dq.titleSwitch': "切换";
    readonly 'dq.titleActive': "展示中";
    readonly 'dq.titleLocked': "未解锁";
    readonly 'dq.share': "分享卡片";
    readonly 'dq.shareDone': "卡片已生成并下载";
    readonly 'dq.shareFailed': "卡片生成失败";
    readonly 'dq.celebration': "🎉 里程碑达成！";
    readonly 'dq.celebrationLevel': "升至 Lv.{level} · {title}";
    readonly 'dq.celebrationStats': "本等级用时 {days} 天 · {turns} 回合";
    readonly 'dq.calendar': "活跃日历";
    readonly 'dq.calendarDays': "近 30 天";
    readonly 'dq.stats': "统计";
    readonly 'dq.statsBestCombo': "历史最高连击";
    readonly 'dq.statsBestLevel': "历史最高等级";
    readonly 'dq.statsTopTools': "工具使用 TOP5";
    readonly 'dq.statsTurns': "回合";
    readonly 'dq.statsTimeline': "成就解锁时间线";
    readonly 'dq.records': "荣誉墙";
    readonly 'dq.recordsSeason': "赛季 {season}";
    readonly 'dq.recordsCombo': "连击 {combo}";
    readonly 'dq.counters': "计数";
    readonly 'dq.adventure': "冒险事件";
    readonly 'dq.adventureEmpty': "暂无事件——每完成 20 个回合掷一次命运骰子";
    readonly 'dq.eventTitle': "命运骰子";
    readonly 'dq.eventChoiceA': "选项 A";
    readonly 'dq.eventChoiceB': "选项 B";
    readonly 'dq.buffCount': "生效 {n} 个";
    readonly 'dq.buffActive': "生效中";
    readonly 'dq.stance': "连击姿态";
    readonly 'dq.relics': "圣物收藏";
    readonly 'dq.relicsCount': "{n}/{m}";
    readonly 'dq.relicsEmpty': "还没捡到圣物——全清待办/击败 BOSS 有概率掉落";
    readonly 'dq.chain': "史诗任务";
    readonly 'dq.chainDone': "待领奖";
    readonly 'dq.chainClaim': "🎉 领取终章奖励 +{xp} XP";
    readonly 'dq.chainClaimed': "史诗任务完成 +{xp} XP";
    readonly 'dq.chainClaimedDone': "终章奖励已领取";
    readonly 'dq.ghost': "幽灵竞速";
    readonly 'dq.ghostVs': "vs 上周的你";
    readonly 'dq.ghostTurnsLabel': "回合";
    readonly 'dq.ghostBeaten': "已击败 ✓";
    readonly 'dq.ghostClaim': "领击败奖励";
    readonly 'dq.ghostClaimed': "击败上周的自己 +{xp} XP";
    readonly 'dq.ghostClaimedDone': "奖励已领取";
};
declare const en: Record<keyof typeof zh, string>;
declare module '@deepseek-ai/dsh-client-ui-slots' {
    interface LocaleNamespaceMap {
        'devquest': keyof typeof zh;
    }
}
export { zh, en };
