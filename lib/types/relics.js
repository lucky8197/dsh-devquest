import { hashSeed, relicLuckActive } from "./events.js";
/** 本地日期键 'YYYY-MM-DD'（与 engine.dayKey 一致；保持零依赖避免循环 import）。 */
export function dayKeyLocal(now) {
...
/** 已收集的圣物（id 集合）。 */
export function ownedRelics(save) {
 ...
 */
export function rollRelic(save, chance, now, seed) {
    const s = structuredClone(save);
    const owned = ownedRelics(s);
    if (owned.size >= RELIC_POOL.length)
    ...
    for (const r of candidates) {
        pick -= weight(r);
        if (pick < 0) {
            s.relics = [...(s.relics ?? []), { id: r.id, acquiredAt: now }];
...
/** 条件是否满足（基于当天累计指标）。 */
export function chainStepMet(def, save, now) {
 ...
 */
export function advanceQuestChain(save, now, seed) {
    const s = structuredClone(save);
    const today = dayKeyLocal(now);
    ...
    if (chain.lastProgressDay === today)
        return none; // 今天已推进过
    const stepDef = def.steps[chain.step];
    if (stepDef === undefined)
        return none;
    if (!chainStepMet(stepDef, s, now))
        return none;
...
export function claimChainReward(save, now, seasonOverride) {
    const s = structuredClone(save);
...
export function pastWeekTotals(save, now) {
    let xp = 0;
    let turns = 0;
...
export function thisWeekTotals(save, now) {
    let xp = 0;
    let turns = 0;
    const h = save.history ?? {};
    for (let i = 0; i < 7; i++) {
 ...
 */
export function ensureGhostRace(save, now) {
    const s = structuredClone(save);
    const g = s.ghostRace;
    if (g !== undefined && g.week === 'rolling')
        return s; // 已初始化（滚动窗口语义，无需换周）
    const past = pastWeekTotals(s, now);
    if (past.xp <= 0 && past.turns <= 0)
        return s;
    s.ghostRace = { week: 'rolling', ghostXp: past.xp, ghostTurns: past.turns, claimed: false };
    return s;
}
/** 当前对决进度（client 进度条用）。 */
export function ghostRaceProgress(save, now) {
    const g = save.ghostRace;
    if (g === undefined)
        return { active: false, ghostXp: 0, ghostTurns: 0, myXp: 0, myTurns: 0, beaten: false, claimed: false };
...
export function claimGhostReward(save, now, seasonOverride) {
    const s = structuredClone(save);
    const g = s.ghostRace;
    if (g === undefined || g.claimed === true)
        return { ok: false, gained: 0, save: s };
    const mine = thisWeekTotals(s, now);
    if (mine.xp < g.ghostXp || mine.turns < g.ghostTurns)
        return { ok: false, gained: 0, save: s };
...
/** 梗化标签（确定性抽取；约 60% 用梗版）。 */
export function memedDailyLabel(id, def, seed) {
...
/** 按周种子取 BOSS 花名。 */
export function bossMemeName(seed) {
    return BOSS_MEME_NAMES[hashSeed(`boss-${seed}`) % BOSS_MEME_NAMES.length];
}
/** 幽灵界面花名（本轮对决对手名称）。 */
export function ghostMemeName(seed) {
    const names = [
        { zh: '过去的你', en: 'Past You' },
